(function (global) {
  ...
  function iconFor(type) {
    switch ((type || "").toLowerCase()) {
      ...
      case "question":
        return '<i class="fas fa-question-circle me-2"></i>';
      default:
        return "";
    }
  }
  ...
  function showConfirm(options = {}) {
    try {
      ...
      const modalId =
        "modal-" + Date.now() + "-" + Math.floor(Math.random() * 1000);
      ...
      const dialogClass =
        position === "center"
          ...
          : "modal-dialog";
      const dialogStyle =
        position === "top-middle" ? 'style="margin: 1.5rem auto;"' : "";
      wrapper.innerHTML = `
                  ...
                  <div class="${dialogClass}" ${dialogStyle}>
                    <div class="modal-content">
                      ...
                      <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">${escapeHtml(
                          cancelText
                        )}</button>
                        <button type="button" class="btn ${confirmBtnClass} confirm-btn">${escapeHtml(
        confirmText
      )}</button>
                      </div>
                    </div>
      ...
      modalEl.addEventListener("shown.bs.modal", function () {
        const openModals = document.querySelectorAll(".modal.show").length; // 1,2,...
        const baseModalZ = 1050;
        const baseBackdropZ = 1040;
        const step = 20; // khoảng cách z-index giữa các layer

        // đặt z-index cho modal hiện tại
        modalEl.style.zIndex = baseModalZ + (openModals - 1) * step;

        // backdrop mới sẽ là phần tử modal-backdrop cuối cùng
        const backdrops = document.querySelectorAll(".modal-backdrop");
        if (backdrops.length) {
          backdrops[backdrops.length - 1].style.zIndex =
            baseBackdropZ + (openModals - 1) * step;
        }
      });
  ...
  function showAlert(options = {}) {
    try {
      ...
      const wrapper = document.createElement("div");
      const dialogClassAlert =
        position === "center"
          ? "modal-dialog modal-dialog-centered"
          : "modal-dialog";
      const dialogStyleAlert =
        position === "top-middle" ? 'style="margin: 1.5rem auto;"' : "";
      wrapper.innerHTML = `
                <div class="modal fade" id="${modalId}" tabindex="-1" aria-labelledby="${modalId}-label" aria-hidden="true">
                  <div class="${dialogClassAlert}" ${dialogStyleAlert}>
                    <div class="modal-content">
                      ...
                      <div class="modal-footer">
                        <button type="button" class="btn btn-primary" data-bs-dismiss="modal">${escapeHtml(
                          buttonText
                        )}</button>
                      </div>
                    </div>
